...
@Component
public class WriteRelationship implements ExportWriter {
	private static final Logger LOGGER = LoggerFactory.getLogger(WriteRelationship.class);

	private final DataStore dataStore;

	public WriteRelationship(DataStore dataStore) {
	...
	public boolean write(ExportConfiguration exportConfiguration) {
		...
		Path fullPath = Paths.get(FileNameService.getRelationship(rf2Package, RF2.FULL, shortName, effectiveTime));
		Path snapshotPath = Paths.get(FileNameService.getRelationship(rf2Package, RF2.SNAPSHOT, shortName, effectiveTime));
		...
		try (BufferedWriter fullWriter = initBufferedWriter(full, fullPath); BufferedWriter snapshotWriter = initBufferedWriter(snapshotPath)) {
			writeHeader(full, RF2.HEADER_RELATIONSHIP, fullPath, snapshotPath);

			for (Map.Entry<String, Set<Relationship>> entrySet : dataStore.readRelationships(sort).entrySet()) {
				...
				if (full) {
					...
					List<Relationship> relationships = new ArrayList<>(value);
					if (sort) {
			...
			LOGGER.error("Failed to write to Relationship file.", e);
			return false;
		}
