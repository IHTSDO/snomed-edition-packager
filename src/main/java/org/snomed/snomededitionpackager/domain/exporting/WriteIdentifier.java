...
import org.snomed.snomededitionpackager.domain.rf2.Identifier;
...
@Component
public class WriteIdentifier implements ExportWriter {
	private static final Logger LOGGER = LoggerFactory.getLogger(WriteIdentifier.class);

	private final DataStore dataStore;

	public WriteIdentifier(DataStore dataStore) {
	...
	public boolean write(ExportConfiguration exportConfiguration) {
		...
		Path fullPath = Paths.get(FileNameService.getIdentifier(rf2Package, RF2.FULL, shortName, effectiveTime));
		Path snapshotPath = Paths.get(FileNameService.getIdentifier(rf2Package, RF2.SNAPSHOT, shortName, effectiveTime));
		...
		try (BufferedWriter fullWriter = initBufferedWriter(full, fullPath); BufferedWriter snapshotWriter = initBufferedWriter(snapshotPath)) {
			writeHeader(full, RF2.HEADER_IDENTIFIER, fullPath, snapshotPath);

			for (Map.Entry<String, Set<Identifier>> entrySet : dataStore.readIdentifiers(sort).entrySet()) {
				Set<Identifier> value = entrySet.getValue();
				...
				if (full) {
					...
					List<Identifier> identifiers = new ArrayList<>(value);
					if (sort) {
					...
					for (Identifier identifier : identifiers) {
						fullWriter.write(identifier.toRF2() + RF2.LINE_ENDING);
					}
				}
			}
		} catch (Exception e) {
			LOGGER.error("Failed to write to Identifier file.", e);
			return false;
		}
