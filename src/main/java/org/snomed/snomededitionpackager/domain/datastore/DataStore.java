...
public class DataStore extends ComponentStore {
	 ...
	 * @return ReferenceSetMembers from store.
	 */
	public Map<String, Set<ReferenceSetMember>> readReferenceSetMembers(boolean sort) {
		return sort ? sortUUID(referenceSetMembers) : referenceSetMembers;
	}
	...
	private <T> Map<String, Set<T>> sortUUID(Map<String, Set<T>> input) {
		return input.entrySet().stream().sorted(Comparator.comparing(e -> UUID.fromString(e.getKey()))).collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue, (oldValue, newValue) -> oldValue, LinkedHashMap::new));
	}
}
